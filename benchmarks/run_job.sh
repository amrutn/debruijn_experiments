...
#SBATCH --mem=224G
#SBATCH --time=24:00:00                # check the partition's max walltime; the job is resumable (see below)
...
cd "$HOME/ngram_experiments/benchmarks"                  # directory containing entropy_exp.py


export HF_HOME=/ceph/projects/pratikac/cot-reasoning/benchmark/hf
...
python entropy_exp.py \
    --experiment knockout-generation \
    --model Qwen/Qwen3-32B \
    --gen-batch-size 64 \
    --gen-full-batch-size 12 \
    --gen-max-new-tokens 12800
