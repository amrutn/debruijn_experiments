...
def parse_args():
    ...
    p.add_argument('--gen-random-ns', type=str, default="1000,n75",
                   help="comma-separated budgets n for the RANDOM-SUBSET control: at "
                        "every decoding step the model attends to the prompt plus a "
                        "freshly drawn uniform-random subset of n tokens of its own "
                        "reasoning (the current token + n-1 earlier ones) instead of "
                        "the n most recent. Accuracies and lengths are stored in the "
                        "same cache and reported next to the recency window at the "
                        "same n. The entry 'n75' means, per model and benchmark, the "
                        "smallest swept n whose recency accuracy reaches 75%% of "
                        "full-context accuracy (resolved from the cached sweep, so the "
                        "sweep and full column must be computed first; done in the "
                        "same run if not). Needs the full KV cache, so it uses "
                        "--gen-full-batch-size. Pass '' to disable.")
    # ---- knockout experiment ----
...
ARGS = parse_args()
if ARGS.devices:
    # accept "cuda:0 cuda:1", "0 1", and comma-joined forms like "cuda:0,cuda:1"
    _tokens = [t for d in ARGS.devices for t in str(d).split(',') if t.strip()]
    _indices = [t.strip()[len('cuda:'):] if t.strip().lower().startswith('cuda:') else t.strip()
                for t in _tokens]
...
def load_model(attn_implementation=None):
    ...
    if DEVICE == "cuda" and torch.cuda.device_count() > 1:
        ...
        #
        # On a SINGLE GPU this path is skipped and we fall through to device_map="auto",
        # which places the whole model on cuda:0 (e.g. 32B ~64 GiB fits an 80/180 GiB
        # H100/B200 with room to spare).
        #
        # low_zero must stay False: it gives GPU 0 a budget of max(0, model_size -
        # capacity of the OTHER GPUs), so on 2 x 32 GiB with a 27.5 GiB 14B model GPU 0
        # gets nothing, the whole model lands on GPU 1, and the first forward OOMs
        # even at batch 4. Even balancing leaves the same headroom on every GPU.
        try:
            ...
            bal = get_balanced_memory(
                probe, dtype=kwargs['dtype'], low_zero=False,
...
# Random-subset control. Same memory BUDGET as the recency window at size n, but
# instead of the n most recent tokens the model sees, at every decoding step, a
# freshly drawn uniform-random subset of its own reasoning: the current token plus
# n-1 of the previously generated tokens (and always the whole prompt, with true
# position ids, exactly as in the knockout). It separates "how much context" from
# "which context": if recency is what matters, this control should do worse than
# the recency window at the same n. Its results live in the same per-problem cache
# under the column key -n (a sentinel, like GEN_FULL_N for full context).
def rand_col(n):
    return -int(n)
...
def col_label(n):
    """Display label for a cache column key."""
    n = int(n)
    return "full" if n == GEN_FULL_N else (f"rand{-n}" if n < 0 else str(n))
...
def knockout_generate_one(model, tokenizer, prompt_ids, n, args, eos_ids,
                          random_subset=False, subset_seed=0):
    """
    ...
    random_subset=True is the control: instead of the n most recent tokens, each
    step sees the current token plus a freshly drawn uniform-random n-1 of the
    earlier generated tokens (same budget n, same true position ids).
    """
    device = model.device
    P = len(prompt_ids)
    generated = []
    rng = np.random.default_rng(subset_seed) if random_subset else None
    for _ in range(args.gen_max_new_tokens):
        G = len(generated)
        if random_subset and G > n:
            # the last token is the query and is always kept; the rest are random
            sel = sorted(rng.choice(G - 1, size=n - 1, replace=False).tolist()) + [G - 1]
        else:
            w = min(n, G) if n > 0 else 0
            sel = list(range(G - w, G))                 # the n most recent (or all, if fewer)
        window = [generated[i] for i in sel]
        input_ids = torch.tensor([prompt_ids + window], device=device)
        pos = list(range(P)) + [P + i for i in sel]     # TRUE positions of the visible tokens
...
def _random_subset_mask(B, t, k, device, rng):
    """
    (B, t) 0/1 key mask over the t previously generated tokens that selects,
    independently for every row, a uniform-random subset of min(k, t) of them.
    The top-k of iid uniform scores is a uniform random k-subset.
    """
    if k <= 0 or t == 0:
        return torch.zeros((B, t), dtype=torch.long, device=device)
    if t <= k:
        return torch.ones((B, t), dtype=torch.long, device=device)
    scores = torch.rand((B, t), generator=rng, device=device)
...
def knockout_generate_batch(model, tokenizer, prompts_ids, n, args, eos_ids, pad_id,
    ...
    random_subset=True is the random-subset CONTROL at the same budget n: nothing
    is evicted (any earlier token may be needed later), and at every step a fresh
    key mask exposes the prompt + a uniform-random n-1 of the generated tokens +
    the current token, drawn independently per sequence. Its KV cache grows like
    the full-context column, so call it with the full-column batch size.
    ...
    if random_subset:
        ...
        rng.manual_seed(int(subset_seed))
    B = len(prompts_ids)
    ...
    for _ in range(args.gen_max_new_tokens):
        ...
        if random_subset:
            # control: the cache holds ALL win_len generated tokens; expose a fresh
            # random n-1 of them + the new token itself (= budget n), per sequence
            gen_mask = _random_subset_mask(B, win_len, n - 1, device, rng)
            full_mask = torch.cat([amask, gen_mask,
                                   torch.ones((B, 1), dtype=torch.long, device=device)], dim=1)
        else:
            full_mask = torch.cat([amask, torch.ones((B, win_len + 1), dtype=torch.long,
                                                     device=device)],
        ...
        win_len += 1
        if random_subset:
            continue                                    # never evict: win_len = all generated
...
def _model_style_legend(ax, primary_label, cmp_label, loc, bbox_to_anchor=None, ncol=1,
                        handlelength=1.8):
    ...
    ax.legend(handles=handles, loc=loc, frameon=False, handlelength=handlelength, ncol=ncol,
              columnspacing=0.5, handletextpad=0.35, borderaxespad=0.0, **kw)

def plot_knockout_generation_results(results, n_values, results_cmp=None,
                                     primary_label=None, cmp_label=None):
    """Accuracy vs memory window n, one line per benchmark with a +/-1 SEM band
    (no grid). If `results_cmp` is given, that model is overlaid as dashed curves
    (hollow 'full' markers) in the same per-benchmark colors."""
    print("Generating Knockout-Generation Accuracy Plot...")
    plt.rcParams.update({'font.size': 12, 'axes.labelsize': 14, 'axes.titlesize': 16,
                         'xtick.labelsize': 12, 'ytick.labelsize': 12, 'legend.fontsize': 8})
    fig, ax = plt.subplots(figsize=(3, 2.5))
    ...
    def _draw(res, color, dashed):
        ...
        ax.fill_between(x, acc - sem, acc + sem, color=color, alpha=0.18 if dashed else 0.3,
                        linewidth=0, zorder=2)                        # +/-1 SEM band
    ...
    from matplotlib.lines import Line2D
    # short benchmark names here ('GPQA', not 'GPQA-Diamond') so the model legend fits beside it
    bench = [Line2D([0], [0], color=c, ls='-', lw=1.8, label=nm)
             for nm, c in styles.items() if nm in results]
    # lower-right corner, dipping just below the spine with tight row spacing so the
    # legend's top stays under the GPQA curve's SEM band (~0.27 at n>=400)
    leg_bench = ax.legend(handles=bench, loc='lower right', frameon=False, handlelength=1.0,
                          labelspacing=0.25, bbox_to_anchor=(1.0, -0.03), borderaxespad=0.0)
    if results_cmp:
        ax.add_artist(leg_bench)
        # one row in the lower-left corner: the only in-axes spot at (3, 2.5) with no curve
        _model_style_legend(ax, primary_label, cmp_label, loc='lower left',
                            bbox_to_anchor=(0.0, -0.01), ncol=2, handlelength=1.5)    # one character right of the earlier -0.03
    plt.tight_layout()
    os.makedirs(FIGURES_DIR, exist_ok=True)
...
def plot_knockout_generation_lengths(table, n_values):
    ...
    answers (one figure per benchmark), with a 'full'-context
...
def plot_knockout_generation_lengths_combined(table, n_values, table_cmp=None,
    ...
    benchmark on a single axis with a +/-1 SEM band (no grid), and a 'full'-context
    point at the right end. Restricting to correct answers avoids the non-terminating
...
FRACTION_BINS = 10      # quantile (equal-count) bins for the memory-fraction re-binning

def rebin_accuracy_by_fraction(slug, name, n_bins=FRACTION_BINS, min_cells=20):
    """
    Re-index one benchmark's cached knockout-generation cells by the window
    expressed as a FRACTION of the reasoning generated in that cell: f = n / L_i(n),
    with L_i(n) the length problem i's trace reached at window n (cached). f is
    capped at 1: once n >= L_i(n) nothing was ever evicted, so all such cells are the
    same "whole trace in memory" condition. The cells are then quantile-binned
    (n_bins equal-count bins over f; tied f-values are never split across bins, so
    the f = 1 mass forms a single bin) and returns (rows, full_acc, full_sem,
    n_problems), with rows = [(bin_median_f, accuracy, sem, n_cells)] for bins
    holding >= min_cells.

    This needs no new generation, but it is an approximation: a problem only lands
    at the fractions the fixed n-sweep produces, so high-f bins are enriched for
    short-trace (easier) problems, and L_i(n) is itself an outcome of the knockout.
    The unbiased version runs each problem at windows f * L_i directly.
    """
    d = np.load(gen_path_for(slug, name), allow_pickle=False)
    correct, lengths = d['correct'], d['lengths']
    cns = [int(x) for x in d['n_values']]
    sweep = [n for n in cns if 0 < n < GEN_FULL_N]          # recency windows only
    jf = cns.index(GEN_FULL_N)
    ok = correct[:, jf] >= 0
    fr, co = [], []
    for n in sweep:
        j = cns.index(n)
        m = ok & (correct[:, j] >= 0) & (lengths[:, j] > 0)
        fr.append(np.minimum(n / lengths[m, j].astype(float), 1.0))
        co.append(correct[m, j].astype(float))
    fr, co = np.concatenate(fr), np.concatenate(co)
    # quantile edges; np.unique collapses duplicate edges produced by ties (e.g. f = 1)
    edges = np.unique(np.quantile(fr, np.linspace(0.0, 1.0, n_bins + 1)))
    idx = np.digitize(fr, edges[1:-1], right=True)         # bin b holds edges[b] < f <= edges[b+1]
    rows = []
    for b in range(len(edges) - 1):
        sel = idx == b
        if sel.sum() >= min_cells:
            p = float(co[sel].mean())
            rows.append((float(np.median(fr[sel])), p,
                         float(np.sqrt(p * (1 - p) / sel.sum())), int(sel.sum())))
    fa = float(correct[ok, jf].mean())
    return rows, fa, float(np.sqrt(fa * (1 - fa) / ok.sum())), int(ok.sum())

def plot_accuracy_vs_fraction_rebinned(model_ids, order=("GSM8K", "MATH-500", "GPQA")):
    """
    Accuracy vs memory window as a fraction of the reasoning length in each cell,
    quantile-binned from the cached results (see rebin_accuracy_by_fraction for the
    caveat). Same styling as plot_knockout_generation_results: one curve per
    benchmark, first model solid and the second dashed. No 'full' point: the
    fraction axis ends at 1 (whole trace in memory), which already is that condition.
    Generated with the other knockout-generation panels (also via --plot-only).
    """
    print("Generating Accuracy-vs-Fraction (re-binned) Plot...")
    plt.rcParams.update({'font.size': 12, 'axes.labelsize': 14, 'axes.titlesize': 16,
                         'xtick.labelsize': 12, 'ytick.labelsize': 12, 'legend.fontsize': 8})
    fig, ax = plt.subplots(figsize=(3, 2.5))
    styles = {'GSM8K': '#003366', 'MATH-500': '#2ca02c', 'GPQA': '#d62728'}
    series = []                                   # (model_id, dashed, name, rows, fa, fs)
    for k, model_id in enumerate(model_ids):
        slug = model_slug(model_id)
        for name in order:
            if os.path.exists(gen_path_for(slug, name)):
                rows, fa, fs, nprob = rebin_accuracy_by_fraction(slug, name)
                series.append((model_id, k > 0, name, rows, fa, fs))
                print(f"  {model_short(model_id)} {name}: {nprob} problems, {len(rows)} bins plotted")
    if not series:
        print("  no cached results found; nothing to plot"); return
    xs_all = np.array([r[0] for _, _, _, rows, _, _ in series for r in rows])
    for model_id, dashed, name, rows, _, _ in series:
        color = styles[name]
        xs = np.array([r[0] for r in rows]); ps = np.array([r[1] for r in rows])
        ss = np.array([r[2] for r in rows])
        ax.plot(xs, ps, color=color, linestyle='--' if dashed else '-', linewidth=1.8, zorder=3)
        ax.fill_between(xs, ps - ss, ps + ss, color=color, alpha=0.18 if dashed else 0.3,
                        linewidth=0, zorder=2)                        # +/-1 SEM band
    ax.set_xlabel("Memory Fraction")
    ax.set_ylabel("Accuracy")
    ax.set_ylim(bottom=0.0, top=1.0)
    _setup_memory_log_xaxis(ax, xs_all, None, False)   # decade ticks only, no 'full' tick
    # no legend: colours/linestyles match the accuracy-vs-memory panel, which carries it
    plt.tight_layout()
    os.makedirs(FIGURES_DIR, exist_ok=True)
    output_path = os.path.join(FIGURES_DIR, "knockout_generation_accuracy_vs_fraction")
    plt.savefig(output_path + ".pdf", dpi=300, bbox_inches="tight")
    plt.savefig(output_path + ".png", dpi=300, bbox_inches="tight")
    print(f"Saved plot to {output_path}")
    plt.close()
...
N75_FRAC = 0.75          # 'n75' = first swept n with recency accuracy >= N75_FRAC * full

def parse_rand_specs(spec):
    """Parse --gen-random-ns into fixed ints plus symbolic entries (only 'n75')."""
    fixed, symbolic = set(), []
    for tok in str(spec).split(','):
        ...
        if tok == 'n75':
            ...
            fixed.add(int(tok))
    return sorted(fixed), symbolic

def n_at_frac_of_full(correct, cns, n_values, nprob, frac=N75_FRAC):
    """
    Smallest n in the recency sweep whose accuracy over the first nprob rows is
    >= frac * full-context accuracy, or None if the sweep/full column is not fully
    computed yet or the threshold is never reached. This is the same rule as the
    'Memory size to exceed 75% of full-context accuracy' table.
    """
    cns = [int(x) for x in cns]
    need = list(n_values) + [GEN_FULL_N]
    if not all(n in cns for n in need):
        return None
    sub = correct[:nprob]
    if (sub[:, [cns.index(n) for n in need]] < 0).any():
        return None                                     # sweep not finished for these rows
    acc, _ = aggregate_gen(sub, cns, n_values)
    full, _ = aggregate_gen(sub, cns, [GEN_FULL_N])
    for n, a in zip(n_values, acc):
        if a >= frac * full[0]:
            return int(n)
    return None

def resolve_rand_ns(rand_fixed, rand_symbolic, correct, cns, n_values, nprob):
    """Concrete random-control budgets for one (model, benchmark): the fixed ones
    plus 'n75' resolved from that benchmark's own cached sweep (dropped if it
    cannot be resolved yet). Returns (sorted ns, n75 or None)."""
    n75 = (n_at_frac_of_full(correct, cns, n_values, nprob)
           if 'n75' in rand_symbolic and correct is not None else None)
    return sorted(set(rand_fixed) | ({n75} if n75 is not None else set())), n75

def report_random_control(model_ids, order, rand_ns, gen_samples, cap,
                          rand_symbolic=(), n_values=()):
    """
    Print and store the random-subset control next to the recency window at the
    same budget n and the full-context reference, for every model in `model_ids`
    that has a cache. Read straight from the cached matrices (no dataset, no GPU),
    so one --plot-only run reports all models. Writes
    knockout_gen/<model>/random_control_summary.csv (one row per condition).

    The same problems are decoded under every condition, so alongside the
    accuracies it prints the paired split: problems solved only with the recency
    window vs only with the random subset.

    'n75' in `rand_symbolic` adds, per model and benchmark, the budget at which the
    recency window first reaches 75% of full-context accuracy (from that model's
    own cached sweep over `n_values`); its rows are tagged window_rule=n75.
    """
    if not rand_ns and not rand_symbolic:
        return
    fnum = lambda v, spec: format(v, spec) if v == v else "n/a"      # NaN-safe
    for model_id in model_ids:
        slug = model_slug(model_id)
        rows_out, printed_header = [], False
        for name in order:
            path = gen_path_for(slug, name)
            if not os.path.exists(path):
                continue
            d = np.load(path, allow_pickle=False)
            cns = [int(x) for x in d['n_values']]
            nprob = min(gen_samples, d['correct'].shape[0])
            correct = d['correct'][:nprob]
            lengths = (d['lengths'][:nprob] if 'lengths' in d.files
                       else np.full(correct.shape, -1, np.int32))
            ns, n75 = resolve_rand_ns(rand_ns, rand_symbolic, correct, cns, n_values, nprob)
            if 'n75' in rand_symbolic and n75 is None:
                print(f"\n  [{model_short(model_id)} {name}] n75 unresolved: recency sweep/full "
                      f"not complete, or 75% of full-context accuracy never reached")
            for n in ns:
                rule = "n75" if n == n75 else "fixed"
                tag = " (n75)" if rule == "n75" else ""
                conds = [("recent", n, n), ("random", n, rand_col(n)), ("full", "", GEN_FULL_N)]
                if not printed_header:
                    print(f"\nRandom-subset control -- {model_id}")
                    print(f"  {'benchmark':<10}{'condition':<20}{'N':>5}{'accuracy':>16}"
                          f"{'len all':>10}{'len correct':>13}{'len incorrect':>15}{'hit cap':>9}")
                    printed_header = True
                for cond, n_disp, key in conds:
                    clabel = cond + (f' n={n_disp}' if n_disp != '' else '') + (tag if n_disp != '' else '')
                    if key not in cns or not (correct[:, cns.index(key)] >= 0).any():
                        print(f"  {name:<10}{clabel:<20}{'-- not computed yet --':>30}")
                        continue
                    j = cns.index(key)
                    a, s = aggregate_gen(correct, cns, [key])
                    st = gen_len_stats(lengths[:, j], correct[:, j], cap)
                    print(f"  {name:<10}{clabel:<20}"
                          f"{st['n']:>5d}{f'{a[0]:.3f} +/- {s[0]:.3f}':>16}"
                          f"{fnum(st['mean'], '.0f'):>10}{fnum(st['mean_correct'], '.0f'):>13}"
                          f"{fnum(st['mean_incorrect'], '.0f'):>15}{st['hit']:>9d}")
                    rows_out.append([model_id, name, cond, n_disp, st['n'],
                                     fnum(float(a[0]), '.4f'), fnum(float(s[0]), '.4f'),
                                     fnum(st['mean'], '.2f'), fnum(st['sem'], '.2f'),
                                     fnum(st['mean_correct'], '.2f'), fnum(st['sem_correct'], '.2f'),
                                     fnum(st['mean_incorrect'], '.2f'),
                                     fnum(st['sem_incorrect'], '.2f'), st['hit'],
                                     rule if n_disp != '' else ""])
                # paired comparison on problems decoded under BOTH conditions
                if n in cns and rand_col(n) in cns:
                    cr, cx = correct[:, cns.index(n)], correct[:, cns.index(rand_col(n))]
                    both = (cr >= 0) & (cx >= 0)
                    if both.any():
                        only_recent = int(((cr == 1) & (cx == 0) & both).sum())
                        only_random = int(((cr == 0) & (cx == 1) & both).sum())
                        print(f"  {'':<10}paired over {int(both.sum())} problems: solved only with "
                              f"recent n={n}: {only_recent}, only with random n={n}: {only_random}")
        if rows_out:
            out = os.path.join(KNOCKOUT_GEN_DIR, slug, "random_control_summary.csv")
            with open(out, "w") as f:
                f.write("model,benchmark,condition,n,n_problems,accuracy,accuracy_sem,"
                        "mean_len_all,sem_len_all,mean_len_correct,sem_len_correct,"
                        "mean_len_incorrect,sem_len_incorrect,hit_cap,window_rule\n")
                for r in rows_out:
...
def run_knockout_generation(tokenizer):
    ...
    rand_ns, rand_sym = parse_rand_specs(ARGS.gen_random_ns)
    # sweep + the full-context reference point + the fixed random-subset control
    # column(s). A symbolic 'n75' control column is per benchmark (resolved from that
    # benchmark's own sweep) and is added on top of these below.
    all_cols = n_values + [GEN_FULL_N] + [rand_col(n) for n in rand_ns]
    order = ["GSM8K", "MATH-500", "GPQA"]
    rand_desc = [str(n) for n in rand_ns] + list(rand_sym)
    print(f"Knockout-generation window sizes n: {n_values} (+ full)"
          + (f" + random-subset control at n={rand_desc}" if rand_desc else ""))
    ...
    def _needs(name):
        ...
        if (corr[:nprob, cols] < 0).any():
            return True
        # sweep + full complete: the per-benchmark n75 control column must be too
        _, n75 = resolve_rand_ns([], rand_sym, corr, cns, n_values, nprob)
        if n75 is None:
            return False                             # threshold never reached -> nothing to run
        return rand_col(n75) not in list(cns) or bool((corr[:nprob, list(cns).index(rand_col(n75))] < 0).any())
    ...
    for name in order:
        ...
        if model is not None and items:
            ...
            def _fill(n):
                ...
                # the random-subset control keeps the whole KV cache (like "full"),
                # so it also needs the smaller batch; `budget` is its positive size n
                is_rand = is_rand_col(n)
                budget = -int(n) if is_rand else int(n)
                bs_n = full_bs if (n == GEN_FULL_N or is_rand) else bs
                label = col_label(n) if (n == GEN_FULL_N or is_rand) else f"n={n}"
                for s0 in tqdm(range(0, len(todo), bs_n), desc=f"{name} {label}", unit="batch"):
                    ...
                    subset_seed = ARGS.gen_seed * 1000003 + idxs[0]   # reproducible subset draws
                    if ARGS.gen_no_cache:
                        pairs = [knockout_generate_one(model, tokenizer, p, budget, ARGS, eos_ids,
                                                       random_subset=is_rand,
                                                       subset_seed=subset_seed + j)
                                 for j, p in enumerate(prompts)]
                        texts = [t for t, _ in pairs]
                        lens = [l for _, l in pairs]
                    else:
                        texts, lens = knockout_generate_batch(model, tokenizer, prompts, budget,
                                                              ARGS, eos_ids, pad_id,
                                                              random_subset=is_rand,
            ...
            for n in all_cols:
                _fill(n)
            # the n75 random control is resolved only now, from this benchmark's
            # completed sweep + full column, then run like any other random column
            _, n75 = resolve_rand_ns([], rand_sym, correct, cns, n_values, nprob)
            if 'n75' in rand_sym:
                if n75 is None:
                    print(f"  {name}: n75 random control skipped (75% of full-context "
                          f"accuracy not reached for n<={max(n_values)}, or sweep incomplete)")
                else:
                    print(f"  {name}: n75 = {n75} -> random-subset control at n={n75}")
                    _ensure_col(rand_col(n75))
                    _fill(rand_col(n75))
    ...
    # ---- summary tables ----
    col_labels = [col_label(n) for n in all_cols]
    ...
    # random-subset control: report + store for this model and the comparison model
    ctrl_models = [MODEL_NAME] + ([ARGS.compare_model] if ARGS.compare_model
                                  and model_slug(ARGS.compare_model) != MODEL_SLUG else [])
    report_random_control(ctrl_models, order, rand_ns, ARGS.gen_samples, ARGS.gen_max_new_tokens,
                          rand_symbolic=rand_sym, n_values=n_values)
    ...
    # re-binned accuracy vs memory-as-a-fraction-of-trace (from the same caches)
    plot_accuracy_vs_fraction_rebinned([MODEL_NAME] + ([ARGS.compare_model] if results_cmp else []),
                                       order=order)
